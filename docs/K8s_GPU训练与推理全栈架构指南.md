...
> 从物理部署到 AI 平台：集群在哪里、GPU 如何被调用、训练和推理工作流、训练与推理的部署差异、AI 平台组件栈、存储架构（含训练/推理存储需求差异）、网络架构（含 PD 分离推理网络需求）、多租户管理
>
> 撰写人：孟希東
...
训练需要高速网络支撑集合通信，这一点没有争议；但"推理不需要高速网络"这个结论在 2026 年已经不完全成立——要看采用的是哪种推理架构。

| 维度 | 训练 | 推理（传统一体化） | 推理（PD 分离，2026 主流） |
|------|------|---------------------|---------------------------|
| 通信模式 | 集合通信（AllReduce，梯度同步） | 几乎无跨节点通信 | 点对点传输（KV Cache 从 Prefill 传到 Decode） |
| 通信频率 | 每个训练 step 都要通信 | 无 | 每个请求一次（Prefill 完成后传一次） |
| 是否需要高速网络 | 是，强制要求 | 否，普通以太网够用 | 是，KV Cache 传输要求 RDMA |
| 延迟容忍度 | 相对宽松（整体训练时长掩盖单次通信延迟） | 无特殊要求 | 严格（目标 < 50ms，否则拖慢首 token 时间） |
| 流量方向 | 东西向（GPU 之间） | 南北向（用户请求进出） | 东西向（Prefill→Decode）+ 南北向 |

**传统一体化推理**：Prefill（处理输入 prompt）和 Decode（逐 token 生成）在同一组 GPU 上完成，请求进出走标准 Service/Ingress，各推理实例完全独立、互不通信，这种架构下确实几乎不需要特殊网络。

**Prefill-Decode 分离架构（PD 分离）**：LLM 推理有两个阶段——计算密集的 Prefill 阶段和显存带宽密集的 Decode 阶段，把这两个阶段拆到不同的 GPU 池上独立优化已成为业界标准做法，vLLM、SGLang、TensorRT-LLM、NVIDIA Dynamo 等主流推理框架均已支持，DeepSeek、Gemini、Meta、LinkedIn、Mistral 等在生产环境大规模部署这一架构。

这个架构给推理引入了训练之外的新网络需求：Prefill 节点处理完 prompt 后，必须把生成的 KV Cache 通过网络传输给 Decode 节点，Decode 才能开始生成，这个传输在两者部署于不同节点时必须足够快，否则会拖慢 Decode 的持续供给。

**关键技术：NIXL**。NVIDIA Inference Xfer Library（NIXL）是当前传输 KV Cache 张量的标准机制，vLLM 和 NVIDIA Dynamo 都用它，支持 RDMA 或 TCP——RDMA（InfiniBand 或 RoCE）可实现亚毫秒级传输，没有 RDMA 则退化到 TCP，延迟更高但仍可用。生产实践中通常用 RDMA 传输、FP8 压缩把 KV Cache 体积减半、并在 Prefill 完全结束前就开始预取传输，目标是把传输时间控制在 50ms 以内。

**带宽量化参考**：以 Llama 3.1 70B、FP8、8K 上下文、batch=8 为例，一个 batch 的 KV Cache 约 10GB，要在 Decode 节点开始生成前传完，需要高带宽链路支撑——这也是生产部署会在 Prefill 和 Decode 节点之间优先配置 RDMA 而非普通以太网的原因。

| | 传统一体化推理 | PD 分离推理 |
|---|---|---|
| 是否需要节点间高速网络 | 不需要 | 需要（Prefill↔Decode 之间） |
| 网络用途 | 无跨节点通信 | KV Cache 张量传输 |
| 推荐网络 | 标准以太网即可 | RDMA（InfiniBand/RoCE），可退化用 TCP |
| 延迟目标 | 无特殊要求 | < 50ms |
| K8s 落地平台 | KServe / vLLM Deployment | vLLM Disaggregated / **llm-d**（Red Hat+IBM+Google 主导，2026年3月 KubeCon 捐给 CNCF 的 K8s 原生分离推理框架） |

### 8.5 落到集群设计的建议

"训练池配 InfiniBand，推理池普通以太网就够"这个结论只在采用传统一体化推理时成立。如果是大模型、长上下文、高并发的生产级推理服务，PD 分离架构会让推理网络需求向训练侧靠拢：

```
采用一体化推理（简单场景，中小规模）：
  推理节点池 → 普通以太网即可，各实例完全独立

采用 PD 分离推理（大模型/高吞吐生产场景）：
  Prefill 节点池 ⇄ RDMA 网络 ⇄ Decode 节点池
  → 网络配置向训练池看齐，需要 InfiniBand/RoCE
  → 但只需 Prefill/Decode 之间互联，不需要像训练那样全 GPU 域全互联
```

不是所有推理场景都需要 RDMA——只有采用 PD 分离架构、追求极致吞吐和长上下文性能的生产部署才需要，判断标准是：是否拆分了 Prefill/Decode 节点池，而不是"训练 vs 推理"这个简单二分。
